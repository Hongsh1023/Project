...
public class ReplyManager {
	...
	public static List<ReplyVo> listReply(int board_no) {
		...
		list = session.selectList("board.listReply",board_no);
		session.close();
		return list;
	}
	...
	public static int getTotalReply(int board_no) {
		int n=0;
		SqlSession session = sqlSessionFactory.openSession();
		n=session.selectOne("board.getTotalReply", board_no);
		session.close();
		return n;
	}
	...
	public static int insertReply(ReplyVo r_vo) {
		...
		re=session.insert("board.getTotalReply", r_vo);
		session.close();
		return re;
	}
	...
	public static int deleteReply(int reply_no) {
		int re=0;
		SqlSession session = sqlSessionFactory.openSession(true);
		re=session.insert("board.deleteReply", reply_no);
		session.close();
		return re;
	}
	...
	public static int getNextReplyNo() {
		int n=0;
		SqlSession session = sqlSessionFactory.openSession();
		n = session.selectOne("board.getNextReplyNo");
		session.close();
		return n;
	}
	...
	public static int updateReply(String reply_content, int reply_no) {
		...
		re=session.insert("board.updateReply", map);
		session.close();
		return re;
	}
